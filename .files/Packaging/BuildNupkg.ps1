...
$solutionName = 'InfinniPlatform.WatcherTools.sln'
...
# Restore packages
& "$nugetPath" restore $solutionName -NonInteractive

# Build the solution
& "${Env:ProgramFiles(x86)}\MSBuild\14.0\bin\msbuild.exe" $solutionName /t:Clean /p:Configuration=$buildMode /verbosity:quiet /consoleloggerparameters:ErrorsOnly
& "${Env:ProgramFiles(x86)}\MSBuild\14.0\bin\msbuild.exe" $solutionName /p:Configuration=$buildMode /verbosity:quiet /consoleloggerparameters:ErrorsOnly
